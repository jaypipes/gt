...
// border width.
func (d *Div) Width() types.Dimension {
	parent := d.Parent()
	if parent == nil {
		return types.Dimension(d.Bounds().Dx())
	}
	...
	paddingHoriz := d.Padding().HorizontalSpace()

	ctx := context.TODO()
	display := d.Display()
	if display != types.DisplayInline && d.HasFixedWidth() {
		fixedWidth := d.FixedWidth() + paddingHoriz + borderHoriz
		gtlog.Debug(
			ctx,
	...
	if wc == nil {
		gtlog.Debug(
			ctx,
			"Div.Width[%s]: display=%s width_constraint=none. "+
				"width is parent inner bounds width of %d",
			d.Tag(), display, parentWidth,
		)
		return parentWidth
	}
...
// OuterHeight returns the height of the Div's outer bounding box.
//
...
func (d *Div) Height() types.Dimension {
	parent := d.Parent()
	if parent == nil {
		return types.Dimension(d.Bounds().Dy())
	}
	parentInner := parent.InnerBounds()
	parentWidth := types.Dimension(parentInner.Dx())
	parentHeight := types.Dimension(parentInner.Dy())
	paddingVert := d.Padding().VerticalSpace()
	...
	ctx := context.TODO()
	display := d.Display()
	if display == types.DisplayBlock && d.HasFixedHeight() {
		fixedHeight := d.FixedHeight() + paddingVert + borderVert
		gtlog.Debug(
			ctx,
